...
import { useAllSchoolsParticipants } from "@/hooks/challenger/useAllSchoolsParticipants";
...
const Dashboard = () => {
  ...
  const schoolIds = useMemo(
    () => sportSchools?.map((school) => school.school_id) ?? [],
    [sportSchools],
  );

  const { participantsBySchool } = useAllSchoolsParticipants({
    schoolIds,
  });
  ...
  useEffect(() => {
    if (!schoolId && userSchoolId) {
      router.replace(`/admin/license?school_id=${userSchoolId}`);
    }
  }, [schoolId, userSchoolId, router]);
    ...
    <div className="space-y-6">
      ...
      {sportSchools && sportSchools.length > 0 ? (
        <Tabs
          ...
          onValueChange={(value) => {
            router.push(`/admin/license?school_id=${value}`);
          }}
          className="w-full"
        >
          <TabsList className="flex w-full overflow-x-auto justify-start mb-6 h-auto gap-2">
            {sportSchools.map((school) => {
              const schoolParticipantsCount =
                participantsBySchool[school.school_id]?.length;
